...
(function () {
  ...
  async function extractSubtitleUrl() {
    // Function to detect platform and extract video ID
    function detectPlatformAndExtractID() {
      ...
      // Check YouTube first
      for (let pattern in youtubePatterns) {
        const match = url.match(youtubePatterns[pattern]);
        if (match) return { platform: "youtube", videoID: match[1] };
      }

      // Check VK
      const vkMatch = url.match(vkPattern);
      if (vkMatch) return { platform: "vk", videoID: vkMatch[1] };

      return null;
    }

    const videoInfo = detectPlatformAndExtractID();
    ...
    if (videoInfo.platform === "youtube") {
      ...
      const playerData = await new Promise((resolve) => {
        const checkForPlayer = () => {
          ...
          if (captionData) {
            ...
            if (fetchedVideoID !== videoInfo.videoID) setTimeout(checkForPlayer, 1000);
      ...
      // Fallback: Poll for the element if not immediately available
      return await new Promise((resolve) => {
        const checkForSubtitle = () => {
          ...
          if (subtitleElement) {
            ...
            setTimeout(checkForSubtitle, 1000); // Retry every second
          }
        };
        checkForSubtitle();
      });
